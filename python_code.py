...
def do_crash(line, line2, i):
	global collide_car, crasheds, crashchannels, lines_in_my_car
	if (calculateIntersectPoint(line[0], line[1], line2[0], line2[1]) != None):
		collide_car = True
		if crasheds[i] >= cars*len(lines_in_my_car):
...
while not done:
	...
	clock.tick(6000)
	for i in range(cars):
		speeds[i] += (size_of_car/100)*(datatups[i][1]/50)
